...
# 初始化每个格子下采用动作的概率
# 4个 12行4列 的矩阵，12行4列 中的4列代表的是每个格子的进行上下左右四个方向动作，
# 所以 [4, 12, 4] 代表的就是 4行12列 的格子地图的每个格子的每个方向的概率
...
def get_qsa(row, col, action):
    ...
    # print(next_row, next_col, value + reward)
    return value + reward
...
def get_pi():
    ...
    for row in range(4):
        for col in range(12):
            ...
            count = (action_value == max(action_value)).sum()
            # 让这些动作均分概率
            for action in range(4):
                if action_value[action] == max(action_value):
